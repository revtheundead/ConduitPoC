...
initialized so that encode() never encounters None mandatory fields.
...
from generated.messages import (
    ...
    Cat007UplinkRecord, Cat007UplinkRecordItems,
...
from generated.structs import (
    ...
    Cat007I440, Cat007I440Registers, Cat007I420, Cat007I170,
...
def rt_safe_u16(rng, scale):
    """Generate a roundtrip-safe unsigned 16-bit raw value for the given scale."""
    while True:
        raw = rng.randint(0, 65535)
        if int(raw * scale / scale) == raw:
            return raw

def rt_safe_u8(rng, scale):
    """Generate a roundtrip-safe unsigned 8-bit raw value for the given scale."""
    while True:
...
def rt_safe_signed(rng, bits, scale):
    """Generate a roundtrip-safe signed raw value for the given scale."""
    lo = -(1 << (bits - 1))
    hi = (1 << (bits - 1)) - 1
    while True:
...
def rand_icao_str(rng):
    ...
    return "".join(rng.choice(_ICAO_CHARS) for _ in range(length))
...
def rand_cat021_i150(rng):
    ...
    it.airSpeed = rand_u16(rng, 0x7FFF)
    return it

def rand_cat021_i090(rng):
    it = Cat021I090()
    it.nucrNacv = rand_u8(rng, 7)
    it.nucpNic = rand_u8(rng, 15)
    return it
...
def rand_fill_cat048_re(rng, re_items):
    ...
    if rand_bool(rng):
        m4e = Cat048ReM4e()
        m4e.foeFri = rand_enum(rng, FoeFriId)
        re_items.m4e = m4e
    if rand_bool(rng):
        rpc = Cat048ReRpc()
        sub = Cat048ReRpcSub()
...
def rand_fill_registers(rng, i440):
    ...
    i440.rep = n
    for _ in range(n):
        elem = Cat007I440Registers()
        elem.bds1 = rand_u8(rng, 15)
        elem.bds2 = rand_u8(rng, 15)
        i440.registers.append(elem)
...
def random_cat007_downlink(rng):
    ...
    if rand_bool(rng): it.i020 = rand_cat007_i020(rng)
    if rand_bool(rng):
        i040 = Cat007I040()
    ...
    if rand_bool(rng):
        i161 = Cat007I161()
        i161.trackNumber = rand_u16(rng, 4095)
        it.i161 = i161
    if rand_bool(rng):
        i042 = Cat007I042()
    ...
    if rand_bool(rng):
        i200 = Cat007I200()
        i200.groundSpeed = rand_u16(rng) * 0.000061
        i200.heading = rand_u16(rng) * 0.0054931640625
        it.i200 = i200
    if rand_bool(rng): it.i170 = rand_cat007_i170(rng)
    if rand_bool(rng):
        i210 = Cat007I210()
        i210.sigmaX = rand_u8(rng) * 0.0078125
        i210.sigmaY = rand_u8(rng) * 0.0078125
        i210.sigmaV = rand_u8(rng) * 0.000061
        i210.sigmaH = rand_u8(rng) * 0.087890625
        it.i210 = i210
    if rand_bool(rng):
        i030 = Cat007I030()
    ...
    if rand_bool(rng):
        i110 = Cat007I110()
        i110.height3d = rand_signed(rng, 14) * 25.0
        it.i110 = i110
...
def random_cat007_uplink(rng):
    rec = Cat007UplinkRecord()
    it = Cat007UplinkRecordItems()
    rec.items = it
    ...
    if rand_bool(rng):
        i420 = Cat007I420()
        i420.rhoStart = rand_u16(rng) * 0.00390625
        i420.rhoEnd = rand_u16(rng) * 0.00390625
        i420.thetaStart = rand_u16(rng) * 0.0054931640625
        i420.thetaEnd = rand_u16(rng) * 0.0054931640625
        it.i420 = i420
    if rand_bool(rng):
        i440 = Cat007I440()
        rand_fill_registers(rng, i440)
        it.i440 = i440
...
def rand_cat253_squitter(rng):
    sq = Cat253Squitter()
    sq.rxChannel = rand_enum(rng, Cat253SqRxChannel)
    sq.msSquitter = rand_enum(rng, Cat253SqMsSquitter)
    sq.m5Squitter = rand_enum(rng, Cat253SqM5Squitter)
    return sq

def rand_cat253_bit_report(rng):
    br = Cat253BitReport()
    br.bitType = rand_enum(rng, Cat253BrBitType)
    br.successState = rand_enum(rng, Cat253BrSuccessState)
    br.antennaGeneral = rand_enum(rng, Cat253BrAntennaGeneral)
    br.antennaScanError = rand_enum(rng, Cat253BrAntennaScanError)
    br.powerSourceGeneral = rand_enum(rng, Cat253BrPowerSourceGeneral)
    br.ctrlUnitGeneral = rand_enum(rng, Cat253BrCtrlUnitGeneral)
    br.kbCryptoComm = rand_enum(rng, Cat253BrKbCryptoComm)
    br.kbRtc = rand_enum(rng, Cat253BrKbRtc)
    br.kbHeatWarning = rand_enum(rng, Cat253BrKbHeatWarning)
    br.bottomBandGeneral = rand_enum(rng, Cat253BrBottomBandGeneral)
    br.tbCtrlComm = rand_enum(rng, Cat253BrTbCtrlComm)
    br.tbConfigUpload = rand_enum(rng, Cat253BrTbConfigUpload)
    br.vswrGeneral = rand_enum(rng, Cat253BrVswrGeneral)
    br.vswrSum = rand_enum(rng, Cat253BrVswrSum)
    br.vswrDiff = rand_enum(rng, Cat253BrVswrDiff)
    br.vswrOmni = rand_enum(rng, Cat253BrVswrOmni)
    br.afbGeneral = rand_enum(rng, Cat253BrAfbGeneral)
    br.afbMainSynth = rand_enum(rng, Cat253BrAfbMainSynth)
    br.afbBitSynth = rand_enum(rng, Cat253BrAfbBitSynth)
    br.afbTotalCh = rand_enum(rng, Cat253BrAfbTotalCh)
    br.afbDiffCh = rand_enum(rng, Cat253BrAfbDiffCh)
    br.afbOmniCh = rand_enum(rng, Cat253BrAfbOmniCh)
    br.rfgybGeneral = rand_enum(rng, Cat253BrRfgybGeneral)
    br.rfgybTotalPwr = rand_enum(rng, Cat253BrRfgybTotalPwr)
    br.rfgybDiffPwr = rand_enum(rng, Cat253BrRfgybDiffPwr)
    br.rfgybOmniPwr = rand_enum(rng, Cat253BrRfgybOmniPwr)
    br.rfgybHeatWarning = rand_enum(rng, Cat253BrRfgybHeatWarning)
    br.cryptoGeneral = rand_enum(rng, Cat253BrCryptoGeneral)
    return br
