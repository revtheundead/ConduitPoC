...
// Dummy ASTERIX Peer -- TCP Server or Client mode (Java 11 / JNI)
//
...
// This is the Java 11 equivalent of xcvr-java/DummyPeer.java, using the JNI backend.
//
// Usage:
...
public class DummyPeer {
    ...
    private static void sendServerMessage(Transceiver tx, Random rng) {
        try {
            switch (rng.nextInt(4)) {
                case 0: {
                    asterix_alt.Cat007DownlinkRecord msg = RandomAsterixAlt.randomCat007Downlink(rng);
                    System.out.println("[SEND] " + asterix_alt.Cat007DownlinkRecord.TYPE_NAME);
                    tx.send(msg);
                    break;
                }
                case 1: {
                    asterix_alt.Cat021Record rec = new asterix_alt.Cat021Record();
                ...
                case 2: {
                    asterix_alt.Cat048Record rec = new asterix_alt.Cat048Record();
                ...
                case 3: {
                    asterix_alt.Cat253Record msg = RandomAsterixAlt.randomCat253(rng);
    ...
    private static void sendClientMessage(Transceiver tx, int peerId, Random rng) {
        try {
            switch (rng.nextInt(4)) {
                case 0: {
                    asterix.Cat007UplinkRecord msg = RandomAsterix.randomCat007Uplink(rng);
                    System.out.println("[SEND] " + asterix.Cat007UplinkRecord.TYPE_NAME);
                    tx.send(peerId, msg);
                    break;
                }
                case 1: {
                    asterix.Cat021Record rec = new asterix.Cat021Record();
                ...
                case 2: {
                    asterix.Cat048Record rec = new asterix.Cat048Record();
                ...
                case 3: {
                    asterix.Cat253Record msg = RandomAsterix.randomCat253(rng);
    ...
    private static void runServer(String[] args, int intervalMs, String logDir, String logPrefix) {
        ...
        try (Transceiver tx = new Transceiver()) {
            ...
            tx.start();
            System.out.println("[dummy_peer] Listening (backend=" + tx.backendName() + "). Press Ctrl+C to stop.");
    ...
    private static void runClient(String[] args, int intervalMs, String logDir, String logPrefix) {
        ...
        try (Transceiver tx = new Transceiver()) {
            ...
            tx.start();
            System.out.println("[dummy_peer] Started (backend=" + tx.backendName() + "). Press Ctrl+C to stop.");
