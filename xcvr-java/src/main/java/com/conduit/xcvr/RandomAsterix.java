 ...
 * items included with ~50% probability.
 *
 * Mirrors xcvr/src/random_asterix.hpp
 */
public final class RandomAsterix {
    ...
    static long randU32(Random rng)           { return randU32(rng, 0xFFFFFFFFL); }
    static int randI8(Random rng)  { return rng.nextInt(256) - 128; }
    static int randI16(Random rng) { return rng.nextInt(65536) - 32768; }
    ...
    static void randFillCat048Re(Random rng, Cat048Record.Items.ReItems re) {
        ...
        if (mode5Pick == 0) {
            ...
            if (randBool(rng)) {
                md5.setTos(-1.0 + rng.nextDouble() * 2.0);
            }
            if (randBool(rng)) {
            ...
            if (randBool(rng)) {
                m5n.setTos(-1.0 + rng.nextDouble() * 2.0);
            }
            if (randBool(rng)) {
            ...
            if (randBool(rng)) {
                m5n.mutableFom().setFom(randU8(rng, 31));
            }
        }
    ...
    static void randFillCat021Re(Random rng, Cat021Record.Items.ReItems re) {
        // BPS (~50%)
        if (randBool(rng)) {
            re.mutableBps().setBps(randU16(rng, 4095) * 0.1);
        }
        ...
        if (randBool(rng)) {
            ...
            if (randBool(rng)) {
                sgv.setHgt(randU8(rng, 127) * 2.8125);
            }
        }
        ...
        // TNH (~50%)
        if (randBool(rng)) {
            re.setTnh(rng.nextDouble() * 360.0);
        }
        ...
        if (randBool(rng) && randBool(rng)) {
            ...
            if (randBool(rng)) {
                mes.mutableFom().setFom(randU8(rng, 31));
            }
            if (randBool(rng)) {
    ...
    static <T> void randFillBds(Random rng, Cat048I250 i250) {
    ...
    static void randFillRegisters(Random rng, Cat007I440 i440) {
        ...
        i440.setRep(n);
        for (int j = 0; j < n; j++) {
            var elem = new Cat007I440.RegistersElement();
            elem.setBds1(randU8(rng, 15));
            elem.setBds2(randU8(rng, 15));
            i440.mutableRegisters().add(elem);
        }
    }
    ...
    public static Cat007UplinkRecord randomCat007Uplink(Random rng) {
        ...
        if (randBool(rng)) it.mutableI200();
        if (randBool(rng)) it.mutableI415();
        if (randBool(rng)) it.mutableI420();
        if (randBool(rng)) randFillRegisters(rng, it.mutableI440());
