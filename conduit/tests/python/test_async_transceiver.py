...
class TestAsyncContextManager:
    @pytest.mark.asyncio
    async def test_async_context_manager(self, mock_transceiver):
    ...
    @pytest.mark.asyncio
    async def test_async_context_manager_stops_if_started(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            await t.start()
...
class TestAsyncLifecycle:
    @pytest.mark.asyncio
    async def test_start_stop(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            await t.start()
            mock_transceiver.start.assert_called_once()
            await t.stop()
    ...
    @pytest.mark.asyncio
    async def test_is_running(self, mock_transceiver):
        async with AsyncTransceiver() as t:
...
class TestAsyncSend:
    @pytest.mark.asyncio
    async def test_send_typed(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            msg = MockMessage(10)
            await t.send(1, msg)
    ...
    @pytest.mark.asyncio
    async def test_send_sole_peer(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            msg = MockMessage(10)
            await t.send(msg)
    ...
    @pytest.mark.asyncio
    async def test_send_raw(self, mock_transceiver):
        async with AsyncTransceiver() as t:
...
class TestAsyncQuery:
    @pytest.mark.asyncio
    async def test_peer_count(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            assert t.peer_count() == 1

    @pytest.mark.asyncio
    async def test_sole_peer(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            assert t.sole_peer() == 1

    @pytest.mark.asyncio
    async def test_peer_by_name(self, mock_transceiver):
        async with AsyncTransceiver() as t:
...
class TestAsyncHandlerRegistration:
    @pytest.mark.asyncio
    async def test_on_typed_handler(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            @t.on(MockMessage)
    ...
    @pytest.mark.asyncio
    async def test_on_raw_handler(self, mock_transceiver):
        async with AsyncTransceiver() as t:
            @t.on(type_id=0x9999)
    ...
    @pytest.mark.asyncio
    async def test_on_invalid_class_raises(self, mock_transceiver):
        async with AsyncTransceiver() as t:
...
class TestAsyncStateCallbacks:
    @pytest.mark.asyncio
    async def test_on_state_change(self, mock_transceiver):
        async with AsyncTransceiver() as t:
    ...
    @pytest.mark.asyncio
    async def test_on_error(self, mock_transceiver):
        async with AsyncTransceiver() as t:
...
class TestMessageStream:
    @pytest.mark.asyncio
    async def test_message_stream_iteration(self):
        queue = asyncio.Queue()
...
class TestDispatchAsync:
    @pytest.mark.asyncio
    async def test_dispatch_sync_handler(self, mock_transceiver):
        received = []
    ...
    @pytest.mark.asyncio
    async def test_dispatch_async_handler(self, mock_transceiver):
        received = []
