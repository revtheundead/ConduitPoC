...
// Conduit - Java Panama FFI codec benchmark
//
// Mirrors the C++ benchmark suite using PanamaCodecBinding (JDK 21+ FFI).
// Requires: libconduit_codec_cabi_bench on java.library.path
//
// Usage:
//   java --enable-preview --enable-native-access=ALL-UNNAMED \
//        -Djava.library.path=lib -cp <classpath> BenchPanama

import io.conduit.PanamaCodecBinding;
import io.conduit.NativeCodecBinding;
import io.conduit.NativeCodecBinding.DecodedMessage;

public class BenchPanama {
    ...
    public BenchPanama() {
        binding = new PanamaCodecBinding();
    ...
    public void runCodecRoundtrip() {
        System.out.println("\nCodec: roundtrip (Java encode → Panama decode)");
        System.out.println("-------------------------------------------");
        for (int i = 0; i < BenchWireData.COUNT; i++) {
            final int idx = i;
            bench("roundtrip " + BenchWireData.LABELS[i], () -> {
                byte[] wire = wireData[idx];
    ...
    public void runStress() {
        ...
        System.out.println("\nStress: decode 100K (Java Panama FFI)");
        System.out.println("-------------------------------------------");
        for (int idx : idxs) {
        ...
        System.out.println("\nStress: roundtrip 100K (Java encode → Panama decode)");
        System.out.println("-------------------------------------------");
        for (int j = 0; j < idxs.length; j++) {
    ...
    public void runMixed() {
        System.out.println("\nStress: mixed workload 100K (Java Panama FFI)");
        System.out.println("-------------------------------------------");
    ...
    public void runThroughput() {
        ...
        System.out.println("\nThroughput: batch decode 10K (Java Panama FFI)");
        System.out.println("-------------------------------------------");
        for (int idx : idxs) {
    ...
    public static void main(String[] args) {
        System.out.println("============================================================");
        System.out.println("Conduit Java Panama FFI Benchmark (JDK 21+)");
        System.out.println("============================================================");

        BenchPanama b = new BenchPanama();
        b.bootstrap();
        ...
        System.out.println("\n============================================================");
        System.out.println("Java Panama FFI benchmark complete.");
        System.out.println("============================================================");
    }
}
