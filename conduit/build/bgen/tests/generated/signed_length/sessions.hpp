...
// Session for frame: SignedFrame
class SignedFrameSession : public conduit::traits::ISession {
public:
    [[nodiscard]] conduit::Result<std::vector<conduit::traits::DecodedMessage>>
    decode_frame(std::span<const uint8_t> data) override {
        auto frame = SignedFrame::decode_bytes(data);
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0xf53d748c36a47245ULL) {
            auto* msg = std::any_cast<Payload>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for Payload"));
            auto frame = SignedFrame::wrap(*msg);
    ...
    [[nodiscard]] std::string_view type_name(uint64_t type_id) const override {
        switch (type_id) {
            case 0xf53d748c36a47245ULL: return "Payload";
        }
        return "unknown";
    }
    ...
    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0xf53d748c36a47245ULL) {
            auto* m = std::any_cast<Payload>(&payload);
...
inline std::unique_ptr<conduit::traits::ISession> create_signed_frame_session() {
    return std::make_unique<SignedFrameSession>();
}
