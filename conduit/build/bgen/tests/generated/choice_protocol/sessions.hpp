...
public:
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0x24395aaf5388c853ULL) {
            auto* msg = std::any_cast<AlphaBody>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for AlphaBody"));
            auto frame = Frame::wrap(*msg);
            return frame.encode_bytes();
        } else if (type_id == 0x8b968b89bfd16bffULL) {
            auto* msg = std::any_cast<BetaBody>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for BetaBody"));
            auto frame = Frame::wrap(*msg);
            return frame.encode_bytes();
        } else {
    ...
    [[nodiscard]] std::span<const uint64_t> leaf_type_ids() const override {
        static constexpr uint64_t ids[] = {
            0x24395aaf5388c853ULL, // AlphaBody
            0x8b968b89bfd16bffULL // BetaBody
        };
        return ids;
    }
    ...
    [[nodiscard]] std::string_view type_name(uint64_t type_id) const override {
        switch (type_id) {
            case 0x24395aaf5388c853ULL: return "AlphaBody";
            case 0x8b968b89bfd16bffULL: return "BetaBody";
        }
        return "unknown";
    }
    ...
    [[nodiscard]] bool is_receive_only(uint64_t type_id) const override {
        switch (type_id) {
            case 0x8b968b89bfd16bffULL: return true; // BetaBody
        }
        return false;
    }
    ...
    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0x24395aaf5388c853ULL) {
            auto* m = std::any_cast<AlphaBody>(&payload);
            return m ? m->to_string() : std::string{};
        } else if (type_id == 0x8b968b89bfd16bffULL) {
            auto* m = std::any_cast<BetaBody>(&payload);
            return m ? m->to_string() : std::string{};
        }
        return {};
    }
