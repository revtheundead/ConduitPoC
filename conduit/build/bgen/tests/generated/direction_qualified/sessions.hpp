...
public:
    ...
    decode_frame(std::span<const uint8_t> data) override {
        ...
        for (const auto& dm : messages) {
            if (dm.type_id == 0x43eb816dac19f83eULL) {
                LOG_WARNF("Received send-only message type '{}' (type_id=0x{:016x})", "UplinkPayload", 0x43eb816dac19f83eULL);
            }
        }
        return messages;
    }
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0x43eb816dac19f83eULL) {
            auto* msg = std::any_cast<UplinkPayload>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for UplinkPayload"));
            auto frame = Frame::wrap(*msg);
            return frame.encode_bytes();
        } else if (type_id == 0x1b0d6d132ec96e89ULL) {
            auto* msg = std::any_cast<DownlinkPayload>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for DownlinkPayload"));
            auto frame = Frame::wrap(*msg);
            return frame.encode_bytes();
        } else if (type_id == 0xd99b6c6485bfb782ULL) {
            auto* msg = std::any_cast<CommonPayload>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for CommonPayload"));
            auto frame = Frame::wrap(*msg);
            return frame.encode_bytes();
        } else {
    ...
    [[nodiscard]] std::string_view type_name(uint64_t type_id) const override {
        switch (type_id) {
            case 0x43eb816dac19f83eULL: return "UplinkPayload";
            case 0x1b0d6d132ec96e89ULL: return "DownlinkPayload";
            case 0xd99b6c6485bfb782ULL: return "CommonPayload";
        }
        return "unknown";
    }
    ...
    [[nodiscard]] bool is_receive_only(uint64_t type_id) const override {
        switch (type_id) {
            case 0x1b0d6d132ec96e89ULL: return true; // DownlinkPayload
        }
        return false;
    }
    ...
    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0x43eb816dac19f83eULL) {
            auto* m = std::any_cast<UplinkPayload>(&payload);
            return m ? m->to_string() : std::string{};
        } else if (type_id == 0x1b0d6d132ec96e89ULL) {
            auto* m = std::any_cast<DownlinkPayload>(&payload);
            return m ? m->to_string() : std::string{};
        } else if (type_id == 0xd99b6c6485bfb782ULL) {
            auto* m = std::any_cast<CommonPayload>(&payload);
            return m ? m->to_string() : std::string{};
        }
        return {};
    }
