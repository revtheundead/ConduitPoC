...
// Session for frame: ArithFrame
class ArithFrameSession : public conduit::traits::ISession {
public:
    [[nodiscard]] conduit::Result<std::vector<conduit::traits::DecodedMessage>>
    decode_frame(std::span<const uint8_t> data) override {
        auto frame = ArithFrame::decode_bytes(data);
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0x1863c5954592f1a2ULL) {
            auto* msg = std::any_cast<DataMsg>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for DataMsg"));
            auto frame = ArithFrame::wrap(*msg);
...
inline std::unique_ptr<conduit::traits::ISession> create_arith_frame_session() {
    return std::make_unique<ArithFrameSession>();
}
