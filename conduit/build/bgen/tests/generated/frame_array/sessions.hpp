...
// Session for frame: ArrayFrame
class ArrayFrameSession : public conduit::traits::ISession {
public:
    [[nodiscard]] conduit::Result<std::vector<conduit::traits::DecodedMessage>>
    decode_frame(std::span<const uint8_t> data) override {
        auto frame = ArrayFrame::decode_bytes(data);
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0x8a4ff2bd9c13498cULL) {
            auto* msg = std::any_cast<Record>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for Record"));
            auto frame = ArrayFrame::wrap(*msg);
    ...
    encode_batch(uint64_t type_id, std::span<const std::any> payloads) override {
        if (type_id == 0x8a4ff2bd9c13498cULL) {
            ArrayFrame frame;
            frame.set_msg_type(Record::ID_VALUE);
            frame.payload().reserve(payloads.size());
            for (const auto& p : payloads) {
                auto* msg = std::any_cast<Record>(&p);
                if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                    "payload type mismatch for Record"));
    ...
    [[nodiscard]] std::string_view type_name(uint64_t type_id) const override {
        switch (type_id) {
            case 0x8a4ff2bd9c13498cULL: return "Record";
        }
        return "unknown";
    }
    ...
    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0x8a4ff2bd9c13498cULL) {
            auto* m = std::any_cast<Record>(&payload);
...
inline std::unique_ptr<conduit::traits::ISession> create_array_frame_session() {
    return std::make_unique<ArrayFrameSession>();
}
