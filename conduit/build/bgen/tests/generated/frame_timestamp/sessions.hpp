...
public:
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0xe9bee119c6a37b89ULL) {
            ...
            auto frame = TsFrame::wrap(*msg);
            frame.set_ts(static_cast<uint32_t>(static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count()) & 0xffffffffULL));
...
inline std::unique_ptr<conduit::traits::ISession> create_ts_frame_session() {
    return std::make_unique<TsFrameSession>();
}
