...
public:
    ...
    decode_frame(std::span<const uint8_t> data) override {
        auto frame = SimpleFrame::decode_bytes(data);
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0xbb06e4bc7e4e3277ULL) {
            ...
            auto frame = SimpleFrame::wrap(*msg);
            return frame.encode_bytes();
        } else if (type_id == 0xad3e3c8146fc920fULL) {
            auto* msg = std::any_cast<Status>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for Status"));
            auto frame = SimpleFrame::wrap(*msg);
            return frame.encode_bytes();
        } else {
    ...
    [[nodiscard]] std::string_view type_name(uint64_t type_id) const override {
        switch (type_id) {
            ...
            case 0xad3e3c8146fc920fULL: return "Status";
        }
        return "unknown";
    }
    ...
    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0xbb06e4bc7e4e3277ULL) {
            ...
            auto* m = std::any_cast<Status>(&payload);
            return m ? m->to_string() : std::string{};
        }
        return {};
    }
...
inline std::unique_ptr<conduit::traits::ISession> create_simple_frame_session() {
    return std::make_unique<SimpleFrameSession>();
}
