...
// Session for frame: DirFrame
class DirFrameSession : public conduit::traits::ISession {
public:
    [[nodiscard]] conduit::Result<std::vector<conduit::traits::DecodedMessage>>
    decode_frame(std::span<const uint8_t> data) override {
        auto frame = DirFrame::decode_bytes(data);
        ...
        for (const auto& dm : messages) {
            if (dm.type_id == 0x690fe6e07a2be67fULL) {
                LOG_WARNF("Received send-only message type '{}' (type_id=0x{:016x})", "CommandRequest", 0x690fe6e07a2be67fULL);
            }
        }
        return messages;
    }
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0x690fe6e07a2be67fULL) {
            auto* msg = std::any_cast<CommandRequest>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for CommandRequest"));
            auto frame = DirFrame::wrap(*msg);
            return frame.encode_bytes();
        } else if (type_id == 0x47d59132f22db045ULL) {
            auto* msg = std::any_cast<CommandResponse>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for CommandResponse"));
            auto frame = DirFrame::wrap(*msg);
            return frame.encode_bytes();
        } else if (type_id == 0xbb06e4bc7e4e3277ULL) {
            auto* msg = std::any_cast<Heartbeat>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
    ...
    [[nodiscard]] std::string_view protocol_name() const override {
        return "frame_direction";
    }

    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0x690fe6e07a2be67fULL) {
            auto* m = std::any_cast<CommandRequest>(&payload);
            return m ? m->to_string() : std::string{};
        } else if (type_id == 0x47d59132f22db045ULL) {
            auto* m = std::any_cast<CommandResponse>(&payload);
            return m ? m->to_string() : std::string{};
        } else if (type_id == 0xbb06e4bc7e4e3277ULL) {
            auto* m = std::any_cast<Heartbeat>(&payload);
            return m ? m->to_string() : std::string{};
        }
        return {};
    }
...
inline std::unique_ptr<conduit::traits::ISession> create_dir_frame_session() {
    return std::make_unique<DirFrameSession>();
}
