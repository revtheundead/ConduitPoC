...
// Session for frame: ConfigFrame
class ConfigFrameSession : public conduit::traits::ISession {
public:
    struct Config {
        uint8 system_id{};
    };

    explicit ConfigFrameSession(const Config& config) : config_(config) {}

    [[nodiscard]] conduit::Result<std::vector<conduit::traits::DecodedMessage>>
    decode_frame(std::span<const uint8_t> data) override {
        auto frame = ConfigFrame::decode_bytes(data);
    ...
    encode_wrap(uint64_t type_id, const std::any& payload) override {
        if (type_id == 0xe9bee119c6a37b89ULL) {
            ...
            auto frame = ConfigFrame::wrap(*msg);
            frame.set_system_id(config_.system_id);
            return frame.encode_bytes();
        } else if (type_id == 0xb346db19a7144b77ULL) {
            auto* msg = std::any_cast<Pong>(&payload);
            if (!msg) return std::unexpected(conduit::Error(conduit::ErrorCode::InvalidArgument,
                "payload type mismatch for Pong"));
            auto frame = ConfigFrame::wrap(*msg);
            frame.set_system_id(config_.system_id);
            return frame.encode_bytes();
        } else {
    ...
    [[nodiscard]] std::string_view type_name(uint64_t type_id) const override {
        switch (type_id) {
            ...
            case 0xb346db19a7144b77ULL: return "Pong";
        }
        return "unknown";
    }
    ...
    [[nodiscard]] std::string format_message(uint64_t type_id, const std::any& payload) const override {
        if (type_id == 0xe9bee119c6a37b89ULL) {
            ...
            auto* m = std::any_cast<Pong>(&payload);
            return m ? m->to_string() : std::string{};
        }
        return {};
    }
...
inline std::unique_ptr<conduit::traits::ISession> create_config_frame_session(const ConfigFrameSession::Config& config) {
    return std::make_unique<ConfigFrameSession>(config);
}
