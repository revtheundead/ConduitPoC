...
std::string generate_j_bit_reader(const std::string& pkg) {
    ...
    ctx.indent();
    ctx.line("byte[] b = new byte[4];");
    ctx.line("for (int i=0;i<4;i++) b[i]=(byte)readBits(8);");
    ...
    ctx.indent();
    ctx.line("byte[] b = new byte[8];");
    ctx.line("for (int i=0;i<8;i++) b[i]=(byte)readBits(8);");
    ...
    ctx.line("if (exp == 0) {");
    ctx.indent();
    ctx.line("if (mant == 0) return Float.intBitsToFloat(sign);");
    ctx.line("exp = 1;");
    ctx.line("while ((mant & 0x0400) == 0) { mant <<= 1; exp--; }");
    ctx.line("mant &= ~0x0400;");
    ctx.line("exp += (127 - 15);");
    ctx.line("return Float.intBitsToFloat(sign | (exp << 23) | (mant << 13));");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.indent();
    ctx.line("byte[] b = new byte[6];");
    ctx.line("for (int i=0;i<6;i++) b[i]=(byte)readBits(8);");
    ctx.line("if (!bigEndian) {");
    ctx.indent();
    ctx.line("for (int i=0;i<3;i++) { byte t=b[i]; b[i]=b[5-i]; b[5-i]=t; }");
    ctx.dedent();
    ctx.line("}");
    ctx.line("long raw = 0;");
    ctx.line("for (int i=0;i<6;i++) raw = (raw<<8)|(b[i]&0xFF);");
    ...
    ctx.line("if (enc == 2) {");
    ctx.indent();
    ctx.line("for (int i=0;i<b.length;i++) b[i]=EBCDIC_TO_ASCII[b[i]&0xFF];");
    ctx.dedent();
    ctx.line("} else if (enc == 1) {");
    ctx.indent();
    ctx.line("for (int i=0;i<b.length;i++) b[i]=(byte)(b[i]&0x7F);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("return new String(b, StandardCharsets.ISO_8859_1);");
    ctx.dedent();
    ...
    ctx.line("for (int i=0;i<count;i++) {");
    ctx.indent();
    ctx.line("int c=(int)readBits(charBits);");
    ctx.line("if (charBits<7) sb.append((char)(c==0?0:(c<32?c+0x40:c)));");
    ctx.line("else sb.append((char)c);");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.line("for (int i=0;i<maxLen;i++) {");
    ctx.indent();
    ctx.line("int c=(int)readBits(8);");
    ctx.line("if (c==terminator) break;");
    ctx.line("sb.append((char)c);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("return sb.toString();");
    ctx.dedent();
    ...
    ctx.indent();
    ctx.line("int c=(int)readBits(8);");
    ctx.line("if (prev==0x0D && c==0x0A) { sb.deleteCharAt(sb.length()-1); break; }");
    ctx.line("sb.append((char)c);");
    ctx.line("prev=c;");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.indent();
    ctx.line("long raw = readBits(bits);");
    ctx.line("long result = 0;");
    ctx.line("long mult = 1;");
    ctx.line("for (int i = 0; i < bits/4; i++) {");
    ctx.indent();
    ctx.line("result += ((raw >> (i*4)) & 0xF) * mult;");
    ctx.line("mult *= 10;");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.indent();
    ctx.line("long sign = readBits(1);");
    ctx.line("long val = readBcd(bits-1);");
    ctx.line("return sign != 0 ? -val : val;");
    ctx.dedent();
    ctx.line("}");
...
std::string generate_j_bit_writer(const std::string& pkg) {
    ...
    ctx.line("public void writeU16(int v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[2];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putShort((short) v);");
    ctx.line("for (byte x : b) writeBits(x & 0xFF, 8);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeU32(int v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[4];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putInt(v);");
    ctx.line("for (byte x : b) writeBits(x & 0xFF, 8);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeU64(long v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[8];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putLong(v);");
    ctx.line("for (byte x : b) writeBits(x & 0xFF, 8);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeF16(float v, boolean be) {");
    ctx.indent();
    ctx.line("int fb = Float.floatToRawIntBits(v);");
    ctx.line("int sign = (fb >> 16) & 0x8000;");
    ctx.line("int exp = ((fb >> 23) & 0xFF) - 127 + 15;");
    ctx.line("int mant = fb & 0x007FFFFF;");
    ctx.line("int h;");
    ctx.line("if (((fb >> 23) & 0xFF) == 255) {");
    ctx.indent();
    ctx.line("h = sign | 0x7C00 | (mant != 0 ? 0x0200 : 0);");
    ctx.dedent();
    ctx.line("} else if (exp >= 31) {");
    ctx.indent();
    ctx.line("h = sign | 0x7C00;");
    ctx.dedent();
    ctx.line("} else if (exp <= 0) {");
    ctx.indent();
    ctx.line("if (exp < -10) {");
    ctx.indent();
    ctx.line("h = sign;");
    ...
    ctx.line("} else {");
    ctx.indent();
    ctx.line("mant |= 0x00800000;");
    ctx.line("int shift = 1 - exp;");
    ctx.line("h = sign | (mant >> (13 + shift));");
    ctx.dedent();
    ctx.line("}");
    ctx.dedent();
    ctx.line("} else {");
    ctx.indent();
    ctx.line("h = sign | (exp << 10) | (mant >> 13);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("if (be) {");
    ctx.indent();
    ctx.line("writeBits((h >> 8) & 0xFF, 8);");
    ctx.line("writeBits(h & 0xFF, 8);");
    ctx.dedent();
    ctx.line("} else {");
    ctx.indent();
    ctx.line("writeBits(h & 0xFF, 8);");
    ...
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeF32(float v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[4];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putFloat(v);");
    ctx.line("for (byte x : b) writeBits(x & 0xFF, 8);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeF48(double v, boolean be) {");
    ctx.indent();
    ctx.line("long raw = Double.doubleToRawLongBits(v) >>> 16;");
    ctx.line("byte[] b = new byte[6];");
    ctx.line("for (int i = 5; i >= 0; i--) {");
    ctx.indent();
    ctx.line("b[i] = (byte) (raw & 0xFF);");
    ctx.line("raw >>>= 8;");
    ctx.dedent();
    ctx.line("}");
    ctx.line("if (!be) {");
    ...
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeF64(double v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[8];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putDouble(v);");
    ctx.line("for (byte x : b) writeBits(x & 0xFF, 8);");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.line("if (enc == 2) {");
    ctx.indent();
    ctx.line("for (int i = 0; i < b.length; i++) b[i] = ASCII_TO_EBCDIC[b[i] & 0xFF];");
    ctx.dedent();
    ctx.line("} else if (enc == 1) {");
    ctx.indent();
    ctx.line("for (int i = 0; i < b.length; i++) b[i] = (byte) (b[i] & 0x7F);");
    ...
    ctx.line("for (int i = 0; i < count; i++) {");
    ctx.indent();
    ctx.line("int c = i < s.length() ? (s.charAt(i) & 0xFF) : pad;");
    ctx.line("if (charBits < 7 && c >= 'a' && c <= 'z') c -= 32;");
    ctx.line("writeBits(c >= 0x40 ? c - 0x40 : c, charBits);");
    ctx.dedent();
    ctx.line("}");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.indent();
    ctx.line("for (int i = 0; i < s.length(); i++) writeU8(s.charAt(i) & 0xFF);");
    ctx.line("writeU8(0x0D);");
    ctx.line("writeU8(0x0A);");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.indent();
    ctx.line("int digits = bits / 4;");
    ctx.line("long limit = 1;");
    ctx.line("for (int i = 0; i < digits; i++) limit *= 10;");
    ctx.line("if (Math.abs(val) >= limit) throw new ConduitCodecException(\"BCD value \" + val + \" exceeds capacity of \" + digits + \" digits\");");
    ctx.line("long raw = 0;");
    ctx.line("long v = Math.abs(val);");
    ctx.line("for (int i = 0; i < bits / 4; i++) {");
    ctx.indent();
    ctx.line("raw |= (v % 10) << (i * 4);");
    ctx.line("v /= 10;");
    ctx.dedent();
    ctx.line("}");
    ...
    ctx.indent();
    ctx.line("int digits = (bits - 1) / 4;");
    ctx.line("long limit = 1;");
    ctx.line("for (int i = 0; i < digits; i++) limit *= 10;");
    ctx.line("if (Math.abs(val) >= limit) throw new ConduitCodecException(\"BCD signed value \" + val + \" exceeds capacity of \" + digits + \" digits\");");
    ctx.line("writeBits(val < 0 ? 1 : 0, 1);");
    ctx.line("writeBcd(Math.abs(val), bits - 1);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void writeSignMagnitude(long val, int bits) {");
    ctx.indent();
    ctx.line("writeBits(val < 0 ? 1 : 0, 1);");
    ctx.line("writeBits(Math.abs(val), bits - 1);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public int sizeBytes() { return (bitPos + 7) / 8; }");
    ctx.line("public byte[] toBytes() { return Arrays.copyOf(buf, sizeBytes()); }");
    ctx.line("public void patchU8(int off, int v) {");
    ctx.indent();
    ctx.line("if (off < buf.length) buf[off] = (byte) (v & 0xFF);");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void patchU16(int off, int v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[2];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putShort((short) v);");
    ctx.line("for (int i = 0; i < 2 && off + i < buf.length; i++) buf[off + i] = b[i];");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void patchU32(int off, int v, boolean be) {");
    ctx.indent();
    ctx.line("byte[] b = new byte[4];");
    ctx.line("ByteBuffer.wrap(b).order(be ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN).putInt(v);");
    ctx.line("for (int i = 0; i < 4 && off + i < buf.length; i++) buf[off + i] = b[i];");
    ctx.dedent();
    ctx.line("}");
    ctx.line("public void alignTo(int boundary) {");
    ctx.indent();
...
std::string generate_j_frame_class(const analyzer::SessionInfo& si,
    ...
    auto emit_j_wrap_frame_fields = [&](const std::vector<model::StructChild>& children,
                                         bool copy_from_msg) {
        for (const auto& child : children) {
            if (auto* f = std::get_if<model::Field>(&child)) {
                std::string member = j_field(f->name);
                if (f->constraint && f->constraint->equals) {
                    ctx.line("frame." + member + " = " + j_qualify_const(*f->constraint->equals) + ";");
                } else if (f->auto_expr) {
    ...
    for (const auto& lt : si.leaf_types) {
        ...
        emit_j_wrap_frame_fields(frame.header_fields, true);
