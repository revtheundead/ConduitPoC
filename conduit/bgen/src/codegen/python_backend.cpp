...
void emit_py_decode_children(EmitContext& ctx, const std::vector<model::StructChild>& children,
    ...
    for (size_t idx = 0; idx < children.size(); ++idx) {
        ...
        if (auto_length_idx >= 0 && static_cast<int>(idx) == auto_length_idx
            ...
            ctx.line("_auto_len_consumed = _auto_len_start - r.remaining_bytes()");
            ctx.line("_auto_len_total = int(" + raw_len + ")");
            ctx.line("_auto_len_avail = r.remaining_bytes()");
            ctx.line("_auto_len_remaining = _auto_len_total - _auto_len_consumed");
            ctx.line("if _auto_len_remaining < 0:");
            ctx.indent();
            ctx.line("warnings.warn(f\"decode: auto-length field '" + auto_length_field_name
                     + "' total={_auto_len_total} consumed={_auto_len_consumed} available={_auto_len_avail}; "
                     "decoded without length boundary\")");
            ctx.dedent();
            ctx.line("else:");
            ctx.indent();
            ctx.line("try:");
            ctx.indent();
            ctx.line("r = r.sub_reader(_auto_len_remaining)");
            ctx.dedent();
            ctx.line("except DecodeError:");
            ctx.indent();
            ctx.line("warnings.warn(f\"decode: auto-length field '" + auto_length_field_name
                     + "' total={_auto_len_total} consumed={_auto_len_consumed} available={_auto_len_avail}; "
                     "decoded without length boundary\")");
